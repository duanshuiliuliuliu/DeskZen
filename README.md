...
- 文案来源：每状态每天由大模型批量生成一批台词（需配置 Key 并开启开关），优先弹出；取不到（未配 Key / 关闭 / 当日未生成）回退角色自带 `bubbles` 静态池。两路文案都经洗牌袋逐条弹出，一轮之内不重复。
...
3. Rust 组装 system prompt（角色定义 + 回复风格 + 当前状态约束），以 `stream: true` 调用配置的大模型。
