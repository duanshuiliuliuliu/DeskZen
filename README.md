...
设置窗口（480×620，侧边栏收窄到 76px）为侧边栏布局，四个板块：
...
- **日志**：日志级别下拉（trace/debug/info/warn/error，默认 debug）与单个文件大小下拉（1M/5M/10M/30M，默认 10M），
  两者改完即时生效并写入 `prefs.json`；另有「打开日志目录」按钮。日志内容不在界面里展示。
...
排查问题（"动作/台词为什么是这样"）时先看日志：`%LOCALAPPDATA%\com.deskzen.desktop\logs\deskzen.log`，
设置页「日志 → 打开日志目录」可以直接跳过去（界面里不展示日志内容）。

- **级别**：默认 `debug`（排查问题时默认就有细节可看），设置页可切 trace/debug/info/warn/error，改完即时生效；
- **info** 记的是主干：状态切换（含原因：固定时段 / 需求拉取 / 日程循环）、气泡说了哪句、被注意到时的反应与限频、
  每日计划（主题/焦点标签/开口句）、每日气泡生成结果、导入/删除/切换角色；
- **debug** 记的是细节：抽链（被 `when` 挡下的链、命中需求与当日计划偏置的倍率、选中哪条）、每一拍（段、动作、循环数、时长、速度）、
  气泡为什么没说（抑制窗口 / 动作没剩时间 / 没有文案）、大模型请求规模与耗时、生成任务的跳过原因；
- **warn**：角色包被跳过、AI 生成失败、大模型接口报错、托盘菜单添加失败等；
- WebView 前端也进同一份日志：`console.warn/error`、未捕获错误与未处理的 Promise 拒绝都会转发（来源标记 `web`）；
  `console.log/info` 只在 debug 级别下落盘；
- **不记录聊天内容**：大模型调用只记模型名、消息条数/字数、耗时与错误，不记正文；
- 单份文件超过「单个文件大小」设置（1M/5M/10M/30M，默认 10M）就滚成 `deskzen_<时间>.log`，只保留最近 7 份。
