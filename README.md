...
1. 前端点击「📷」→ `chat_send(messages, use_screenshot=true)`。
2. 截图前 Rust 先临时隐藏角色窗与对话窗（避免它们挡住被截内容），再用 `xcap` 抓取角色/对话窗所在显示器，随后恢复窗口显示。
3. 把截图作为 `image_url` 追加到最后一条用户消息（多模态 `content` 数组），并用视觉模型（`vision_model`）调用。
4. 回复显示在对话窗；若问题为空，默认询问“请看看我当前屏幕上的内容”。

> 截图问答需模型支持图像输入，默认视觉模型为 `deepseek-v4-flash-vision-exp`，可在设置界面修改。
...
- **大模型设置**：接口地址、模型、视觉模型（截图问答用）、API Key、保存按钮与保存状态。
