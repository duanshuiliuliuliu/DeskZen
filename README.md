...
| 场景编排（scenes） | 每个状态配一个加权场景池，一个场景可由多个 `{clip,loops}` 步骤组成；**由 Rust 决定播哪个场景**，前端只渲染；同一场景至少播 12 秒（不足则自然循环），且绝不与上一次相同 |
...
- `scenes`：状态 → 微场景池，一个场景由多个 `{clip,loops}` 步骤组成，`weight` 控制随机权重。每次进入状态时按权重挑一个场景播放完整序列（同一场景至少播 12 秒、不与上一次相同，避免动作被自己或快速切换打断）；每个状态都必须至少有一个场景；
