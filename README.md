...
| 编排（chains/scenes） | 状态 → 链（链内**有序**表达因果、链间按权重可选）→ 段（有起止的表现，多步 `{clip,seconds}`）→ 动作实例；**由 Rust 决定走哪条链、播哪一段**，前端只渲染；动作实例连续播 30~60 秒（不足则循环同一动作填满） |
...
| 操作 | 行为 |
|---|---|
| 鼠标移入角色 | 触发一次「注意到你」：挂起当前活动、播一段短反应，随后回到原处继续（同种事件限频 30 秒） |
| 单击角色（未拖动） | 同上（限频 8 秒） |
...
- `scenes`：状态 → **段**列表，一个段由若干 `{clip, seconds, loops}` 步骤组成。`seconds` 是动作实例的目标时长（秒，按动作原生时长取整循环填满，建议 30~60；小于 30 秒视为过渡动作不配台词）；`loops` 是旧格式回退；
...
- `acknowledge`：「被注意到」时播的短反应（反应结束后回到被打断的那一步继续）：
  ...
  "acknowledge": {
    ...
    "cooldown_s": {"hover": 30, "click": 8, "chat": 10, "talk": 8, "default": 15},
